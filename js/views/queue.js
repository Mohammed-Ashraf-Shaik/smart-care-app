(function () {
    ...
    window.App.Views.Queue = function () {
        ...
        workspaceNav.innerHTML = `<a href="${overviewRoute}" data-route="${overviewRoute}">${icon('layout-dashboard', 16)}<span>${overviewLabel}</span></a>${isStaff ? `<a href="/dashboard/admin/rooms" data-tab="rooms" data-tab-route="/dashboard/admin">${icon('door-open', 16)}<span>Rooms</span></a>` : ''}<a class="active" href="/dashboard/queue" data-route="/dashboard/queue">${icon('list-ordered', 16)}<span>Queue</span></a><a href="/dashboard/analytics" data-route="/dashboard/analytics">${icon('bar-chart-3', 16)}<span>Analytics</span></a><a href="${helpRoute}" data-route="${helpRoute}">${icon('circle-help', 16)}<span>Help</span></a><button type="button" id="workspace-logout" class="signout-btn">${icon('log-out', 16)}<span>Sign out</span></button>`;
        ...
        const renderRows = () => {
            ...
            const filtered = sortQueue(state.queue.filter(patient => { const text = `${patient.name || ''} ${patient.problem || patient.symptoms || ''} ${patient.hospital || ''}`.toLowerCase(); return (!query || text.includes(query)) && (priority === 'all' || patient.triage === priority); }));
            ...
            region.innerHTML = `<div class="queue-table-wrap"><table class="queue-table"><thead><tr><th>Patient</th><th>Reason for visit</th><th>Priority</th><th>Status</th><th>Centre</th><th>Action</th></tr></thead><tbody>${filtered.map((patient, index) => { const [action, label] = nextAction(patient); return `<tr><td><strong>${index + 1}. ${esc(patient.name || 'Patient')}</strong><small>${esc(patient.age || '—')} years · ${esc(patient.gender || 'Not specified')}</small></td><td>${esc(patient.problem || patient.symptoms || 'General consultation')}</td><td><span class="priority-chip ${priorityClass(patient.triage)}">${esc(patient.triage || 'Green')}</span></td><td><span class="queue-status ${statusClass(patient.status)}">${statusLabel(patient.status)}</span></td><td>${esc(patient.hospital || state.loggedHospital || 'Care centre')}<small>${esc(patient.city || state.loggedCity || 'Location pending')}</small></td><td>${action ? `<button class="text-link queue-action" type="button" data-action="${action}" data-id="${esc(patient.id)}">${label}</button>` : `<span class="queue-status">${label}</span>`}</td></tr>`; }).join('')}</tbody></table></div>`;
            region.querySelectorAll('.queue-action').forEach(button => button.onclick = async () => { const patient = state.queue.find(item => String(item.id) === String(button.dataset.id)); if (!patient) return; button.disabled = true; const result = await transitionPatient(patient.id, button.dataset.action); if (!result.success) { button.disabled = false; window.App.UI.toast(result.error, 'error'); return; } window.App.UI.toast(`${patient.name || 'Patient'} is now ${statusLabel(button.dataset.action)}.`, 'success'); });
