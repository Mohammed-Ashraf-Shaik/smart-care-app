    ...
    window.App.Views.Patient = function (isEmbedded = false) {
        ...
        function renderLocation(target) {
            ...
            function renderMap(lat, lng) {
                if (map) map.remove(); map = null; markerNodes = [];
                ...
                if (window.maplibregl) {
                    ...
                    map.addControl(new window.maplibregl.NavigationControl({ showCompass: false }), 'bottom-right'); map.addControl(new window.maplibregl.AttributionControl({ compact: true }), 'bottom-left');
                    ...
                    const userElement = document.createElement('div'); userElement.className = 'smartcare-user-marker'; userElement.innerHTML = '<span></span>'; new window.maplibregl.Marker({ element: userElement }).setLngLat([lng, lat]).setPopup(new window.maplibregl.Popup().setText('Your selected location')).addTo(map);
                    visibleHospitals.forEach((hospital, index) => { const element = document.createElement('button'); element.type = 'button'; element.className = 'smartcare-hospital-marker'; element.setAttribute('aria-label', `Select ${hospital.name}`); element.innerHTML = `<span><b>${index + 1}</b></span>`; const marker = new window.maplibregl.Marker({ element }).setLngLat([hospital.lng, hospital.lat]).setPopup(new window.maplibregl.Popup({ offset: 20 }).setHTML(`<strong>${esc(hospital.name)}</strong><br>${esc(hospital.type || 'Care centre')}<br>${hospital.distance.toFixed(1)} km away`)).addTo(map); element.onclick = () => selectHospital(hospital.id); markerNodes.push({ hospital, marker }); });
                    if (visibleHospitals.length) { const bounds = new window.maplibregl.LngLatBounds([lng, lat], [lng, lat]); visibleHospitals.forEach(hospital => bounds.extend([hospital.lng, hospital.lat])); map.fitBounds(bounds, { padding: 50, maxZoom: 14 }); }
                } else {
