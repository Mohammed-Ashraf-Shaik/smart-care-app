(function () {
    ...
    window.App.Views.Analytics = function () {
        ...
        workspaceNav.innerHTML = `<a href="${overviewRoute}" data-route="${overviewRoute}">${icon('layout-dashboard', 16)}<span>${overviewLabel}</span></a><a href="/dashboard/queue" data-route="/dashboard/queue">${icon('list-ordered', 16)}<span>Queue</span></a><a class="active" href="/dashboard/analytics" data-route="/dashboard/analytics">${icon('bar-chart-3', 16)}<span>Analytics</span></a><a class="${activeTab === 'volume' ? 'active' : ''}" href="/dashboard/analytics?tab=volume" data-tab="volume" data-tab-route="/dashboard/analytics">${icon('activity', 16)}<span>Volume</span></a><a class="${activeTab === 'signals' ? 'active' : ''}" href="/dashboard/analytics?tab=signals" data-tab="signals" data-tab-route="/dashboard/analytics">${icon('signal', 16)}<span>Signals</span></a><button type="button" id="workspace-logout">${icon('log-out', 16)}<span>Sign out</span></button>`;
